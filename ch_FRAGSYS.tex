...
\begin{equation}
    I_{rel} = \frac{\lvert A \cap B\rvert}{\, \, \, \, \, \, \, \, \lvert A \cap B\rvert_{\text{max}}}
...
VarAlign \cite{MACGOWAN_2020_DRSASP} was used to retrieve genetic variants from gnomAD v2.1 \cite{KARCZEWSKI_2020_GNOMAD} found in the human sequences within the multiple sequence alignment generated for each target protein. gnomAD includes exomes and genomes from 141,456 unrelated individuals without known phenotypic conditions, therefore providing a reasonable representation of the general \textit{healthy} population. Variants found in the human sequences within the alignment were mapped to individual columns and missense enrichment scores (MES) were calculated. MES reflects the enrichment in missense variants of an alignment column relative to the average of the the other columns in the alignment \cite{MACGOWAN_2017_VARIANTS, MACGOWAN_2024_VARIANTS}. 95\% confidence intervals (CI) and \textit{p}-values were used to assess the significance of these ratios \cite{SZUMILAS_2010_ODDSRATIOS}. MES was also calculated for the defined ligand binding sites. The MES of a binding site represents the enrichment in missense variants of a binding site ($site$) relative to the rest of protein residues ($rest$). This was calculated by aggregating all the variants found in the residues forming each binding site (\autoref{eq:MES_site}). Alignment columns as well as binding sites were classified as enriched (MES $>$ 0), depleted (MES $<$ 0) or neutral (MES = 0). Enrichment was not calculated for two of the 39 proteins since no human homologues were identified.

\begin{equation}
\text{MES}_{site} = \frac{\text{variants}_{site} /\text{residues}_{site}}{\text{variants}_{rest} /\text{residues}_{rest}}
\label{eq:MES_site}
\end{equation}
\myequations{Binding site missense enrichment score}
...
\endhead
%
\# Neurons (\textit{input})           & \texttt{11}                               \\ \hline
Number of layers             & \texttt{1}                                \\ \hline
Type of layer                & \texttt{Hidden (fully connected)}         \\ \hline
\# Neurons (\textit{hidden})          & \texttt{10}                               \\ \hline
Activation function (\textit{hidden}) & \texttt{ReLU}                             \\ \hline
\# Neurons (\textit{output})          & \texttt{4}                                \\ \hline
Activation function (\textit{output}) & \texttt{Softmax}                          \\ \hline
Loss function                & \texttt{Sparse categorical cross-entropy} \\ \hline
Weight initialiser           & \texttt{Glorot Uniform}                   \\ \hline
Optimiser                    & \texttt{Adam}                             \\ \hline
Learning rate                & \texttt{0.01}                             \\ \hline
Dropout rate                 & --                               \\ \hline
Regularisation               & --                               \\ \hline
Batch size                   & \texttt{32}                               \\ \hline
Validation split             & \texttt{0.2}                              \\ \hline
\# Training epochs           & \texttt{300}                              \\ \hline
\caption[\textit{Current} MLP hyperparameters]{\textbf{\textit{Current} MLP hyperparameters.} Values of the \textit{current} multilayer perceptron hyperparameters. Parameters were optimised to maximise cross-validation accuracy and minimise loss, i.e., sparse categorical cross-entropy. These are the model hyperparameters employed for the final prediction on the blind set.}
\label{tab:HP_current_MLP}\\
\end{longtable}
...
Different loss functions resulted in a substantial loss of accuracy: Δ\textsubscript{\textit{acc}} $\approx$ \textminus50\% (FDR $<$ 0.05). This was expected as they are not appropriate for a multi-label classifier, unlike sparse categorical cross-entropy.
...
Regarding optimisers, they either severely negatively affected accuracy with a Δ\textsubscript{\textit{acc}} $\approx$ \textminus30\% (FDR $<$ 0.05), had no significant effect (FDR $>$ 0.05), or very slightly improved accuracy, such as \texttt{RMSProp} Δ\textsubscript{\textit{acc}} $<$ +1\% (FDR $<$ 0.05).
...
Regarding dropout rates, a rate of 75\% negatively affected prediction with Δ\textsubscript{\textit{acc}} $<$ \textminus2\% (FDR $<$ 0.05). Lower dropout rates: 0.1, 0.25, and 0.33 did improve the accuracy, but the effect size was very small, Δ\textsubscript{\textit{acc}} $<$ +1\% (FDR $<$ 0.05). This result agreed with the effect of the removal of neurons per layer and showed that fewer neurons on a single hidden layer might be enough to achieve a comparable accuracy to the current model, since dropping them out has no effect.
...
\texttt{\textit{N}\textsubscript{LAYERS} = 0}                   & 82.9              &  \textminus11.0                     & 0            \\ \hline
\texttt{\textit{N}\textsubscript{LAYERS} = 2}                    & 92.9              &  \textminus1.0                      & 0            \\ \hline
\texttt{\textit{N}\textsubscript{LAYERS} = 5}                    & 93.8              &  \textminus0.1                   & 0.42         \\ \hline
\texttt{\textit{N}\textsubscript{LAYERS} = 10}                   & 85.0              &  \textminus8.9                   & 0            \\ \hline
\texttt{\textit{N}\textsubscript{NEURONS} = 1}                  & 79.0              &  \textminus14.9                  & 0            \\ \hline
\texttt{\textit{N}\textsubscript{NEURONS} = 2}                  & 89.8               &  \textminus4.2                   & 0            \\ \hline
\texttt{\textit{N}\textsubscript{NEURONS} = 3}                  & 92.9              &  \textminus1.1                   & 0            \\ \hline
\texttt{\textit{N}\textsubscript{NEURONS} = 4}                  & 93.7              &  \textminus0.3                   & 0.24         \\ \hline
\texttt{\textit{N}\textsubscript{NEURONS} = 5}                  & 93.8              &  \textminus0.1                   & 0.54         \\ \hline
\texttt{\textit{N}\textsubscript{NEURONS} = 6}                  & 93.7              &  \textminus0.2                   & 0.26         \\ \hline
\texttt{\textit{N}\textsubscript{NEURONS} = 7}                  & 93.6              &  \textminus0.3                   & 0.08         \\ \hline
...
The focus here is on human proteins, allowing the additional information from human population variation data to be explored. Two protein products of the SARS-CoV-2 replicase polyprotein 1ab (\href{https://www.uniprot.org/uniprotkb/P0DTD1/entry}{P0DTD1}) were removed, since they did not include any human homologues. The remaining 37 protein domains accounted for 1309 three-dimensional structures that included interactions with 1601 ligands of interest, of which 998 were unique. 293 ligand binding sites were defined across these domains, formed by 2664 unique ligand binding residues. The total number of binding sites per domain ranges from 1 to 24, with 33/37 domains presenting more than one defined binding site. The median number of sites per domain is seven.

\autoref{fig:bs_def_examples} illustrates three examples of the 37 proteins for which ligand binding sites were defined by the algorithm presented in this Chapter. The grouping of the ligands into the defined sites reflects the similarity between the interaction fingerprints of the ligands with the protein. See \autoref{tab:proteins_names} for the name, gene, organism and sequence length of these proteins.

\begin{figure}[htb!]
    \centering
    \includegraphics[width=\textwidth]{figures/ch_FRAGSYS/PNG/FIG1_bs_examples.png}
    \caption[Ligand clusters defined by the binding site definition algorithm]{\textbf{Ligand clusters defined by the binding site definition algorithm.} A single protein cartoon representation, coloured in white, is shown for simplicity for each example. Ligands are coloured according to the site they bind to. \textbf{(A)} There were 110 structures depicting human tyrosine-protein phosphatase non-receptor type 1 (\href{https://www.uniprot.org/uniprotkb/P18031/entry}{P18031}) binding 143 ligand molecules, 104 of which were unique. 18 binding sites were defined. Cartoon PDB: \href{https://www.ebi.ac.uk/pdbe/entry/pdb/5qdu}{5QDU} \cite{KEEDY_2018_PTP1B}; \textbf{(B)} The 68 ligands, 30 of which were unique, found across 50 structures of the chestnut blight fungus endothiapepsin (\href{https://www.uniprot.org/uniprotkb/P11838/entry}{P11838}) were classified into 12 distinct binding sites. Cartoon PDB: \href{https://www.ebi.ac.uk/pdbe/entry/pdb/5r1y}{5R1Y} \cite{WOLLENHAUPT_2020_F2X}; \textbf{(C)} For mouse mitogen-activated protein kinase 14 (\href{https://www.uniprot.org/uniprotkb/P47811/entry}{P47811}), 52 structures portrayed the interaction with 53 ligand molecules, 50 of which were unique, which clustered into 10 ligand binding sites. Cartoon PDB: \href{https://www.ebi.ac.uk/pdbe/entry/pdb/1lew}{1LEW} \cite{CHANG_2002_MapKinase}.}
    \label{fig:bs_def_examples}
\end{figure}

\autoref{fig:bss_features} shows that the 293 defined binding sites are diverse in size (number of amino acids), solvent accessibility, evolutionary divergence and missense depletion. The binding site size ranges from 2-40 residues with a median of 9, while the median site RSA ranges from 4-80\%, with a median of 30\%. For evolutionary divergence, the average site \textit{N\textsubscript{Shenkin}} spreads from 0-80, peaking at 40. Lastly, MES spans \textminus0.75 to 1.0, peaking at neutrality (MES $\approx$ 0).
...
% Note: It may be necessary to compile the document several times to get a multi-page table to line up properly
\begin{landscape}
\begin{longtable}{|M{25mm}|M{65mm}|M{25mm}|M{65mm}|M{20mm}|}
\hline
\textbf{UniProt ID} & \textbf{Protein name}                                         & \textbf{Gene name} & \textbf{Organism}                                                     & \textbf{Length} \\ \hline
\endfirsthead
%
\multicolumn{5}{c}%
{{\bfseries Table \thetable} (continued)} \\
...
\endhead
%
\href{https://www.uniprot.org/uniprotkb/H0Y4R8/entry}{H0Y4R8}              & SP100 nuclear antigen                                         & \textit{SP100}     & \textit{Homo sapiens} (Human)                                         & 259             \\ \hline
\href{https://www.uniprot.org/uniprotkb/O15178/entry}{O15178}              & T-box transcription factor T                                  & \textit{TBXT}      & \textit{Homo sapiens}                                                 & 435             \\ \hline
\href{https://www.uniprot.org/uniprotkb/O43809/entry}{O43809}              & Cleavage and polyadenylation specificity factor subunit 5     & \textit{NUDT21}    & \textit{Homo sapiens}                                                 & 227             \\ \hline
\href{https://www.uniprot.org/uniprotkb/O95696/entry}{O95696}              & Bromodomain-containing protein 1                              & \textit{BRD1}      & \textit{Homo sapiens}                                                 & 1058            \\ \hline
\href{https://www.uniprot.org/uniprotkb/P01584/entry}{P01584}              & Interleukin-1 beta                                            & \textit{IL1B}      & \textit{Homo sapiens}                                                 & 269             \\ \hline
\href{https://www.uniprot.org/uniprotkb/P06873/entry}{P06873}              & Proteinase K                                                  & \textit{PROK}      & \textit{Parengyodontium album}                                        & 384             \\ \hline
\href{https://www.uniprot.org/uniprotkb/P0C024/entry}{P0C024}              & Peroxisomal coenzyme A diphosphatase NUDT7                    & \textit{NUDT7}     & \textit{Homo sapiens}                                                 & 238             \\ \hline
\href{https://www.uniprot.org/uniprotkb/P0DTD1/entry}{P0DTD1}              & Replicase polyprotein 1ab                                     & \textit{rep}       & Severe acute respiratory syndrome coronavirus 2 (SARS-CoV-2)         & 7096            \\ \hline
\href{https://www.uniprot.org/uniprotkb/P11838/entry}{P11838}              & Endothiapepsin                                                & \textit{EAPA}      & \textit{Cryphonectria parasitica}                                     & 419             \\ \hline
\href{https://www.uniprot.org/uniprotkb/P15379/entry}{P15379}              & CD44 antigen                                                  & \textit{Cd44}      & \textit{Mus musculus} (Mouse)                                         & 778             \\ \hline
\href{https://www.uniprot.org/uniprotkb/P18031/entry}{P18031}              & Tyrosine-protein phosphatase non-receptor type 1              & \textit{PTPN1}     & \textit{Homo sapiens}                                                 & 435             \\ \hline
\href{https://www.uniprot.org/uniprotkb/P22557/entry}{P22557}              & 5-aminolevulinate synthase, erythroid-specific, mitochondrial & \textit{ALAS2}     & \textit{Homo sapiens}                                                 & 587             \\ \hline
\href{https://www.uniprot.org/uniprotkb/P24821/entry}{P24821}              & Tenascin                                                      & \textit{TNC}       & \textit{Homo sapiens}                                                 & 2201            \\ \hline
\href{https://www.uniprot.org/uniprotkb/P47811/entry}{P47811}              & Mitogen-activated protein kinase 14                           & \textit{Mapk14}    & \textit{Mus musculus}                                                 & 360             \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q12830/entry}{Q12830}              & Nucleosome-remodeling factor subunit BPTF                     & \textit{BPTF}      & \textit{Homo sapiens}                                                 & 3046            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q15047/entry}{Q15047}              & Histone-lysine N-methyltransferase SETDB1                     & \textit{SETDB1}    & \textit{Homo sapiens}                                                 & 1291            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q32ZE1/entry}{Q32ZE1}              & Genome polyprotein                                            & --                 & Zika virus (ZIKV)                                                     & 3419            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q460N5/entry}{Q460N5}              & Protein mono-ADP-ribosyltransferase PARP14                    & \textit{PARP14}    & \textit{Homo sapiens}                                                 & 1801            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q5T0W9/entry}{Q5T0W9}              & Protein FAM83B                                                & \textit{FAM83B}    & \textit{Homo sapiens}                                                 & 1011            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q6B0I6/entry}{Q6B0I6}              & Lysine-specific demethylase 4D                                & \textit{KDM4D}     & \textit{Homo sapiens}                                                 & 523             \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q6PJP8/entry}{Q6PJP8}              & DNA cross-link repair 1A protein                              & \textit{DCLRE1A}   & \textit{Homo sapiens}                                                 & 1040            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q6PL18/entry}{Q6PL18}              & ATPase family AAA domain-containing protein 2                 & \textit{ATAD2}     & \textit{Homo sapiens}                                                 & 1390            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q7LBC6/entry}{Q7LBC6}              & Lysine-specific demethylase 3B                                & \textit{KDM3B}     & \textit{Homo sapiens}                                                 & 1761            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q8IU60/entry}{Q8IU60}              & m7GpppN-mRNA hydrolase                                        & \textit{DCP2}      & \textit{Homo sapiens}                                                 & 420             \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q8WS26/entry}{Q8WS26}              & Farnesyl diphosphate synthase                                 & --                 & \textit{Trypanosoma cruzi}                                            & 425             \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q8WVM7/entry}{Q8WVM7}              & Cohesin subunit SA-1                                          & \textit{STAG1}     & \textit{Homo sapiens}                                                 & 1258            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q8WWQ0/entry}{Q8WWQ0}              & PH-interacting protein                                        & \textit{PHIP}      & \textit{Homo sapiens}                                                 & 1821            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q92835/entry}{Q92835}              & Phosphatidylinositol 3,4,5-trisphosphate 5-phosphatase 1      & \textit{INPP5D}    & \textit{Homo sapiens}                                                 & 1189            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q96HY7/entry}{Q96HY7}              & 2-oxoadipate dehydrogenase complex component E1               & \textit{DHTKD1}    & \textit{Homo sapiens}                                                 & 919             \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q9BRQ3/entry}{Q9BRQ3}              & Uridine diphosphate glucose pyrophosphatase NUDT22            & \textit{NUDT22}    & \textit{Homo sapiens}                                                 & 303             \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q9UGL1/entry}{Q9UGL1}              & Lysine-specific demethylase 5B                                & \textit{KDM5B}     & \textit{Homo sapiens}                                                 & 1544            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q9UIF8/entry}{Q9UIF8}              & Bromodomain adjacent to zinc finger domain protein 2B         & \textit{BAZ2B}     & \textit{Homo sapiens}                                                 & 2168            \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q9UJM8/entry}{Q9UJM8}              & 2-Hydroxyacid oxidase 1                                       & \textit{HAO1}      & \textit{Homo sapiens}                                                 & 370             \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q9UKK9/entry}{Q9UKK9}              & ADP-sugar pyrophosphatase                                     & \textit{NUDT5}     & \textit{Homo sapiens}                                                 & 219             \\ \hline
\href{https://www.uniprot.org/uniprotkb/Q9Y2J2/entry}{Q9Y2J2}              & Band 4.1-like protein 3                                       & \textit{EPB41L3}   & \textit{Homo sapiens}                                                 & 1087            \\ \hline
\caption[Protein names and identifiers]{\textbf{Protein names and identifiers.} UniProt accession identifiers (ID), protein name, gene name, organism and sequence length, i.e., number of amino acid residues, for each of the 35 unique proteins analysed in this Chapter. Replicase polyprotein 1ab of SARS-CoV-2 (\href{https://www.uniprot.org/uniprotkb/P0DTD1/entry}{P0DTD1}) and Cohesin subunit SA-1 (\href{https://www.uniprot.org/uniprotkb/Q8WVM7/entry}{Q8WVM7}) presented two fragment screening experiments each, targeting different domains of the protein, resulting in the final 37 protein domains.}
\label{tab:proteins_names}\\
\end{longtable}
\end{landscape}
...
\begin{figure}[htb!]
    ...
    \caption[Variation in binding site features]{\textbf{Variation in binding site features.} Distribution of size \textbf{(A)}, median RSA \textbf{(B)}, average \textit{N\textsubscript{Shenkin}} \textbf{(C)} and MES \textbf{(D)} across the 293 binding sites defined across the 37 fragment screening datasets. Black dashed lines indicate the median of each distribution.}
...
Twenty-four sites were defined on the surface of NSP13. Two binding sites were identified as C1: BS6 and BS16 (\autoref{fig:c1_example_2}). Visual inspection showed the two sites to be adjacent with a total of 16 residues. Three fragments bind to the site, which is located in the nucleotide and RNA binding interface of NSP13 between the 1B and 2A domains. This is the region where the 5’ end of the RNA binds \cite{YAN_2020_SARSCOV2}. This pocket is determined to be highly druggable and drugs binding to it might be effective against other coronaviruses, due to the pocket's high amino acid conservation \cite{NEWMAN_2021_SARSCOV2}. This agrees with the results presented here, as this site has an average \textit{N\textsubscript{Shenkin}} = 32 and MES = \textminus0.18. Of the 16 positions in this site, four show high conservation across homologues and missense depletion in human: Pro514 (\textit{N\textsubscript{Shenkin}} = 31, MES = \textminus0.56), Asp534 (\textit{N\textsubscript{Shenkin}} = 9, MES = \textminus0.56), Thr552 (\textit{N\textsubscript{Shenkin}} = 48, MES = \textminus1.87) and His554 (\textit{N\textsubscript{Shenkin}} = 36, MES = \textminus0.85). Thr552 shows lowest missense enrichment (MES = \textminus1.87) and therefore is most likely to have a key function in this protein family.
...
Thirty-three unique ligands across 25 ALAS-E structures were grouped into ten binding sites, only one of which is annotated as functional. Three sites were classified as C1. Two of these are known to be on the interface between subunits, form key interactions to maintain the assembly and are close to the PLP binding site \cite{BAILEY_2020_AMINOLEVULINATE}. However, one (BS1) is not mentioned in the literature. This site is located on a deep pocket at the N-terminal region of the protein structure (\autoref{fig:c1_example_4}). Residues in this site are strongly conserved and depleted in missense variation: \textit{N\textsubscript{Shenkin}} = 29, MES = \textminus0.13. Together, this suggests the site has a functional role in the protein, perhaps as an allosteric regulator or through interaction with a partner such as succinate-CoA ligase, SCS-α, (\href{https://www.uniprot.org/uniprotkb/Q96I99/entry}{Q96I99}) \cite{FURUYAMA_2000_SIDEROBLASTIC}. Out of the 16 residues forming the site, Lys381 (\textit{N\textsubscript{Shenkin}} = 38, MES = \textminus0.94) is the most missense-depleted position in the site and should be considered when growing or optimising a fragment binding to this site.
...
\begin{figure}[htb!]
    ...
    \caption[SARS-CoV-2 MPro fragment screening]{\textbf{SARS-CoV-2 MPro fragment screening.} \textbf{(A)} Twenty-six defined ligand binding sites on the SARS-CoV-2 main protease, MPro, (\href{https://www.uniprot.org/uniprotkb/P0DTD1/entry}{P0DTD1}) from 971 ligands from 511 structures; \textbf{(B)} Five of the 9 C1 sites included the known MPro active site and four known potential allosteric sites \cite{DOUANGAMATH_2020_SARSCOV2, DASGUPTA_2022_ALLOSTERIC}. Surface PDB: \href{https://www.ebi.ac.uk/pdbe/entry/pdb/5R7Y}{5R7Y} \cite{DOUANGAMATH_2020_SARSCOV2}.}
    \label{fig:MPro_showcase}
\end{figure}

As a case study, this method was applied to the SARS-CoV-2 main protease, MPro (\href{https://www.uniprot.org/uniprotkb/P0DTD1/entry}{P0DTD1}). Twenty-six sites were defined from 511 structures, from which 9 were classed as C1, 12 as C2, 3 as C3 and only 2 as C4. Of the 9 C1 sites, one corresponds to the active site and three to allosteric sites 1, 2 and 3 \cite{DASGUPTA_2022_ALLOSTERIC}. A further C1 site is located at the dimer interface and known to be a potential allosteric site \cite{DOUANGAMATH_2020_SARSCOV2} (\autoref{fig:MPro_showcase}). The remaining four C1 sites may be important, but each binds only a single ligand and their function is currently unclear.
...
\href{https://www.uniprot.org/uniprotkb/Q9Y2J2/entry}{Q9Y2J2}     & 13.4 & 43.3     & +0.02  & 0.70  & 21          & 4          & 154, 161, 162, 163, 164, 185, 186, 189, 208, 212, 217, 295, 297, 298, 299, 300, 301, 315, 375, 376, 379 & Calmodulin binding \cite{HAN_2000_CYTOSKELETON}                                          \\ \hline
...
\href{https://www.uniprot.org/uniprotkb/Q8WS26/entry}{Q8WS26}     & 22.1 & 31.0       & +0.18  & 0.58 & 8           & 2          & 308, 312, 315, 316, 320, 324, 384, 423                                                                  & IPP binding \cite{GABELLI_2006_FARNESYL}                                                 \\ \hline
...
\href{https://www.uniprot.org/uniprotkb/P47811/entry}{P47811}     & 17.1 & 55.0       & +0.08  & 0.00    & 19          & 10         & 191, 192, 197, 198, 232, 236, 242, 246, 249, 250, 251, 252, 255, 259, 291, 292, 293, 294, 296           & MAP insert motif, Trp197 pocket \cite{FRANCIS_2013_P38A, NICHOLS_2020_MINING} \\ \hline
...
\caption[Literature supported C1 sites]{\textbf{Literature supported C1 sites.} These are 12 C1 sites with no functional annotations in UniProt, therefore labelled as \textit{unknown function}, for which literature has been found that support their functional relevance. UniProt ID indicates the protein UniProt accession. RSA is the median site RSA (\%). \textit{N\textsubscript{Shenkin}} is the average normalised Shenkin score for the site. MES is the average missense enrichment score for the site. \textit{p} is the \textit{p}-value associated to the site MES. \# aas is the number of residues forming the site. \# ligs is the number of ligands binding to the site. UniProt residue numbers is a list of the UniProt residue numbers of the amino acid residues forming the site. Literature support contains a brief description of the literature-reported site function and references.}
...
\href{https://www.uniprot.org/uniprotkb/Q9UGL1/entry}{Q9UGL1}     & 16.6 & 34.0       & \textminus0.01 & 1.00    & 12          & 3          & 658, 659, 662, 663, 666, 667, 670, 701, 736, 737, 738, 741                                         \\ \hline
...
\href{https://www.uniprot.org/uniprotkb/Q6B0I6/entry}{Q6B0I6}     & 12.2 & 26.0       & \textminus0.06 & 0.84 & 7           & 1          & 44, 199, 275, 276, 297, 300, 303                                                                   \\ \hline
...
\href{https://www.uniprot.org/uniprotkb/P24821/entry}{P24821}     & 14.2 & 24.4     & \textminus0.29 & 0.00    & 15          & 8          & 2010, 2011, 2012, 2025, 2045, 2046, 2047, 2048, 2049, 2050, 2054, 2055, 2056, 2057, 2060           \\ \hline

\caption[Novel C1 sites]{\textbf{Novel C1 sites.} These are 17 C1 sites with no functional annotations in UniProt, therefore labelled as \textit{unknown function}, without any literature support. These sites therefore represent novel predicted functional sites. UniProt ID indicates the protein UniProt accession. RSA (\%) is the median site RSA. \textit{N\textsubscript{Shenkin}} is the average normalised Shenkin score for the site. MES is the average missense enrichment score for the site. \textit{p} is the \textit{p}-value associated to the site MES. \# aas is the number of amino acid residues forming the site. \# ligs is the number of ligands binding to the site. UniProt residue numbers is a list of the UniProt residue numbers of the residues forming the site.}
