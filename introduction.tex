...
It is estimated based on geological \cite{SCHIDLOWSKI_1979_LIFE}, fossil \cite{SCHOPF_2007_LIFE} and phylogenetic \cite{BETTS_2018_LIFE} analyses that the origin of life in \textit{our} planet Earth dates back to 3.7-4.0 billion years ago. Since then, \textit{life} has not just survived, but adapted and evolved to give raise to a gargantuan estimated biodiversity of 8.7 million eukaryotic species \cite{MORA_2011_SPECIES} and upward of 1 trillion microbial species \cite{HUG_2016_SPECIES, LOCEY_2016_SPECIES} with the vast majority of these still to be described \cite{COSTELLO_2013_SPECIES}. Despite the immense variation across species in terms of reproductive strategies, morphological, metabolic, behavioural traits or ecological niche, there is one thing \textit{all} species have in common: nucleic acids \cite{KOONIN_2011_LIFE}. All living organisms rely on nucleic acids, mostly deoxyribonucleic acid (DNA), except for some viruses \cite{KOONIN_2006_VIRUS} and viroids \cite{NAVARRO_2021_VIROIDS} that use ribonucleic acid (RNA), to store their genetic information. This information flows sequentially from DNA to RNA through the process of transcription and from RNA to protein through translation. This flow of molecular information is known as the \textit{Central Dogma of Molecular Biology} \cite{CRICK_1958_DOGMA, CRICK_1970_DOGMA}.
...
Proteins are molecular machines that are involved in virtually all cellular processes including cell division, immune response or metabolism. They result from the process of translation of mRNA. Proteins are natural polymers formed of smaller monomers, called amino acids, linked to each other through peptide bonds. Proteins do not carry their function in isolation, but usually interact with other proteins, nucleic acids, ions or small molecules (\autoref{fig:proteins_interact}). Chemical compounds can be used as drugs to modulate protein function. The amount, localisation, state and interaction with other molecules of proteins is strictly controlled by complex gene regulatory networks, signalling cascades, and environment-dependent conformational changes. Any of these mechanisms failing to work correctly can result in under- or overexpression, hypo- or hypermorphism, truncated, mutated or unfolded proteins, which eventually can lead to disease.

%\vspace{-13pt} % Adjust this value as needed

\begin{figure}[htb!]
    \centering
    \includegraphics[width=\textwidth]{figures/ch_INTRO/PNG/proteins_interact.png}
    \caption[Proteins interact with other molecules]{\textbf{Proteins interact with other molecules.} Proteins interact with other molecules to carry out their function, such as other proteins, nucleic acids, i.e., DNA or RNA, ions or small molecule ligands. \textbf{(A)} Haemoglobin subunit alpha (\href{https://www.uniprot.org/uniprotkb/P69905/entry}{P69905}) interacting with haemoglobin subunit beta (\href{https://www.uniprot.org/uniprotkb/P68871/entry}{P68871}) to form the functional haemoglobin heterotetramer. PDB: \href{https://www.ebi.ac.uk/pdbe/entry/pdb/8FDL}{8FDL} \cite{POWELL_2023_HAEMOGLOBIN}; \textbf{(B)} Zinc finger and BTB domain-containing protein 38 (\href{https://www.uniprot.org/uniprotkb/Q8NAP3/entry}{Q8NAP3}) interacting with DNA. PDB: \href{https://www.ebi.ac.uk/pdbe/entry/pdb/6E94}{6E94} \cite{HUDSON_2018_ZINCFINGER}; \textbf{(C)} Calmodulin-1 (\href{https://www.uniprot.org/uniprotkb/P0DP23/entry}{P0DP23}) interacting with two calcium, Ca\textsuperscript{2+}, (\href{https://www.ebi.ac.uk/pdbe-srv/pdbechem/chemicalCompound/show/CA}{CA}) ions. PDB: \href{https://www.ebi.ac.uk/pdbe/entry/pdb/1CLL}{1CLL} \cite{CHATTOPADHYAYA_1992_CALM1}; \textbf{(D)} Acetylcholinesterase (\href{https://www.uniprot.org/uniprotkb/P22303/entry}{P22303}) binding to donepezil (\href{https://www.ebi.ac.uk/pdbe-srv/pdbechem/chemicalCompound/show/E20}{E20}), which is used to treat symptoms of Alzheimer's disease. PDB: \href{https://www.ebi.ac.uk/pdbe/entry/pdb/4EY7}{4EY7} \cite{CHEUNG_2012_ACHE}.}
    \label{fig:proteins_interact}
\end{figure}
...
There are twenty canonical amino acids that are found in all protein sequences. They receive this name because of their chemical structure, which includes both an amino and carboxylic acid functional groups. \autoref{fig:amino_acid} shows the general structure of an amino acid. A carbon atom -- known as α-carbon (CA) -- is found in the centre and binds covalently to four different groups: an amino group (--NH2), a carboxyl group (--COOH), a hydrogen atom (H) and a side chain (R) that differs across the twenty amino acids residues.
...
\begin{equation}
N_{Shenkin} = 100 \times \frac{V_{Shenkin} - V_{Shenkin_{\text{min}}}}{V_{Shenkin_{\text{max}}} - V_{Shenkin_{\text{min}}}}
...
\autoref{chap:LBS_COMP} and \autoref{chap:LBS_IMPROV} describe the largest benchmark of ligand binding site prediction to date by analysing the performance of thirteen methods using a series of metrics on a brand new reference dataset described in \autoref{chap:LIGYSIS_WEB}, \textit{LIGYSIS}.

\subsection{Protein-ligand interactions}

Experimentally determined structures of protein-ligand complexes, such as those in \autoref{fig:small_molecules}, represent static snapshots of a fundamentally dynamic process. This interaction between a protein and a small molecule ligand typically involves reversible non-covalent forces and can be defined as an equilibrium (\autoref{eq:prot_lig_equilibrium}) between the unbound protein ($P$) and ligand ($L$) species and the bound complex ($PL$). This equilibrium can be described by an association constant, $K_a$. $K_a$ measures the strength of the interaction and is calculated as the ratio between the concentration of the $PL$ complex and the product of the concentrations of the unbound $P$ and $L$ species (\autoref{eq:Association_constant}). Alternatively, this interaction can be described by a dissociation constant, $K_d$, which is the inverse of $K_a$ (\autoref{eq:dissociation_constant}). A lower $K_d$ corresponds to a higher binding affinity, as it indicates that the $PL$ complex is favoured over the unbound $P$ and $L$ species at equilibrium \cite{JENCKS_1981_GIBBS}.
%In the same manner, the predicted ability of a residue or protein to interact with a ligand, i.e., \textit{ligandability}, offers insight into the likelihood of this event, but not its dynamics.
\begin{equation}
P + L \rightleftharpoons PL
\label{eq:prot_lig_equilibrium}
\end{equation}
\myequations{Protein-ligand binding equilibrium}

\vspace{-13pt} % Adjust this value as needed

\begin{equation}
K_a = \frac{[PL]}{[P][L]}
\label{eq:Association_constant}
\end{equation}
\myequations{Association constant}

\vspace{-13pt} % Adjust this value as needed

\begin{equation}
K_a = \frac{1}{K_d} \rightarrow K_d = \frac{[P][L]}{[PL]}
\label{eq:dissociation_constant}
\end{equation}
\myequations{Dissociation constant}

%\vspace{-13pt} % Adjust this value as needed

The thermodynamics of protein-ligand binding can be understood by examining the Gibbs free energy change ($\Delta G$), which quantifies the spontaneity and driving force of the interaction. This free energy is determined by enthalpic and entropic contributions as described in \autoref{eq:gibbs_free_energy}. Here, $\Delta H$ reflects the enthalpy change associated with interactions such as hydrogen bonds, van der Waals forces and electrostatics, while $\Delta S$ captures the change in entropy due to effects such as desolvation and conformational flexibility. This free energy change is also directly linked to the equilibrium binding constants through \autoref{eq:gibbs_free_energy2}, where $R$ is the universal gas constant and $T$ is the absolute temperature in Kelvin. A more negative $\Delta G$ indicates stronger binding affinity, i.e., favourable interaction, corresponding to a higher $K_a$ (or equivalently, a lower $K_d$) \cite{BRONOWSKA_2011_PLIS}.

\begin{equation}
\Delta G = \Delta H - T \Delta S
\label{eq:gibbs_free_energy}
\end{equation}
\myequations{Gibbs free energy}

\vspace{-13pt} % Adjust this value as needed
%\vspace{-13pt} % Adjust this value as needed

\begin{equation}
\Delta G = -RT \ln K_a \rightarrow \Delta G = RT \ln K_d
\label{eq:gibbs_free_energy2}
\end{equation}
\myequations{Gibbs free energy - II}

\vspace{-13pt} % Adjust this value as needed

Even small changes in Gibbs free energy can lead to large differences in binding affinity. At 298 K, a change of just $\approx$1.4 kcal/mol corresponds to a 10-fold change in the $K_d$ and a $\Delta G \approx$\textminus4 kcal/mol results in a $\approx$1000$\times$ improvement in affinity, e.g., improving $K_d$ from 10 μM to 10 nM. This exponential sensitivity means that weak interactions can appear artificially strong in experimental assays if ligand concentration is high. When ligand concentration greatly exceeds $K_d$, even low-affinity binders can occupy a large fraction of the target protein, leading to false positives or non-specific binding events. Thus, interpreting $K_d$ values in the context of ligand concentration is essential to distinguish physiologically relevant binding from experimental artefacts \cite{KUNTZ_1999_MAX_AFFINITY}.
